...
class Scribby {
    ...
    init() {
        ...
        this.svgEl.addEventListener('pointermove', this.eventMove.bind(this), false);
    ...
    eventMove(ev) {
        if (this.path) {
            ...
            if (this.pressureEnabled){
                if (Math.abs(ev.pressure - this.pressure) >0.05){
    ...
    eventDown(ev) {
        ...
        this.path.setAttribute('stroke-width', this.attrStrokeWidth * this.pressure);
    ...
    setPressureEnabled(b){
        this.pressureEnabled = b;
    }
